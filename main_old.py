...
date = '12122021'
tipo_recomendacao = 'Cosine'     # Cosine, Euclidean
mcdm_pairwise_method = 'AHP'        # AHP, Promethee
r_pers = 'most_similar'             # most_similar, half_similar_half_dissimilar
    ...
    for engine in recomm_engines:
        for exc in tqdm(range(n_executions)):
            ...
            for aux in range(n_rec, total_samples_per_rec, n_rec):
                ...
                # Load trained model
                # with open("tunned_models/tuned_model_random.pkl", "rb") as fp:
                ...
                if exc == 0:
                    ...
                    if len(results[alg][engine]['tau']) <= int(total_samples_per_rec / n_rec):
                        try:
                            with open("tunned_models/tuned_model_" + engine + "_" + alg + ".pkl", "rb") as fp:
                                tuned_model = pickle.load(fp)
                        except:
                            pass
                    else:
                    ...
                    with open("tunned_models/tuned_model_"+engine+"_"+alg+'_'+date+".pkl", 'wb') as arq:
                        pickle.dump(tuned_model, arq)
                else:
                    # if the execution is from 2nd on, just read the best model from the execution 0
                    with open("tunned_models/tuned_model_"+engine+"_"+alg+"_"+date+".pkl", "rb") as fp:
                        tuned_model = pickle.load(fp)
...
# Save results
filename = open('tunned_models/results'+'_'+date+'.pkl', 'wb')
